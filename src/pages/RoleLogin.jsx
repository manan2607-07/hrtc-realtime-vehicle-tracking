...
export default function RoleLogin() {
  ...
  const { loginAsCustomer, loginAsDriver, loginAsConductor, loginAsAdmin } = useAuth();
  ...
  const roleCards = [
    {
      id: 'customer',
      icon: '👤',
      title: 'Passenger / Citizen',
      subtitle: 'Track buses, view ETAs, and plan your journey',
      color: '#1A5276',
    },
    {
      id: 'driver',
  ...
  return (
    <div className="login-page">
      <div className="login-page__header">
        ...
        <p className="login-page__subtitle">Himachal Road Transport Corporation — Select your role to continue</p>
      </div>
      ...
      {!activeTab ? (
        <div className="login-page__roles">
          {roleCards.map(card => (
            ...
            >
              ...
              {card.id === 'customer' ? (
                <div className="role-card__action">Enter without login →</div>
              ) : (
                <div className="role-card__action">Secure Login Required →</div>
              )}
            </button>
          ))}
        </div>
        ...
        <div className="login-page__form-container">
          ...
          <div className="login-form" style={{ '--role-color': roleCards.find(r => r.id === activeTab)?.color }}>
            <div className="login-form__header">
              <span className="login-form__icon">{roleCards.find(r => r.id === activeTab)?.icon}</span>
              <h2 className="login-form__title">{roleCards.find(r => r.id === activeTab)?.title} Login</h2>
