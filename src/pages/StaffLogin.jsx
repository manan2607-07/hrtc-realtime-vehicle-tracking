 ...
 * Handles Driver and Conductor login with credential verification.
 */
export default function StaffLogin() {
  const navigate = useNavigate();
  const { loginAsDriver, loginAsConductor } = useAuth();
  ...
  return (
    <div className="login-page">
      ...
      {!activeTab ? (
        <div className="login-page__roles" style={{ maxWidth: '560px' }}>
          {staffRoles.map(card => (
            <button
              key={card.id}
              className="role-card"
